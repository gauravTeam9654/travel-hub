...
const HiddenGemsPage = () => {
  ...
  const fetchTrips = async () => {
    try {
      const querySnapshot = await getDocs(collection(db, "hiddenGems"));
...
const handleImageUpload = async (e) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingTrip) {
        ...
        const tripRef = doc(db, "hiddenGems", editingTrip);
        await updateDoc(tripRef, formattedData);
        alert("Hidden Gem updated successfully!");
        setEditingTrip(null);
      } else {
        // Add new trip
        await addDoc(collection(db, "hiddenGems"), formattedData);
      ...
      alert("Failed to save Hidden Gem");
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Are you sure you want to delete this hidden gem?")) return;
    try {
      await deleteDoc(doc(db, "hiddenGems", id));
  ...
  return (
    ...
    >
      <h1 style={{ textAlign: "center", marginBottom: "30px", color: "#333" }}>
        {editingTrip ? "Edit Hidden Gem" : "Add Hidden Gem Package"}
      </h1>
...
>
  ...
  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
    <div>
      ...
      <input
        name="title"
        placeholder="Enter hidden gem title"
  ...
  <div>
    ...
    <textarea
      name="description"
      placeholder="Write about the hidden gem..."
  ...
  >
    {editingTrip ? "Update Hidden Gem" : "Save Hidden Gem"}
  </button>
</form>
      ...
      <div style={{ marginTop: "50px" }}>
        <h2 style={{ marginBottom: "20px" }}>Existing Hidden Gems</h2>
        {trips.length === 0 ? (
